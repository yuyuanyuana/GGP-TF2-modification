...
class GraphPolynomial(gpflow.kernels.base.Kernel):
    ...
    def Kzz(self, Z):
        cov = (self.variance * (tf.matmul(Z, Z, transpose_b=True)) + self.offset) ** self.degree
        return cov

    def Kzx(self, Z, X):
        X = tf.reshape(tf.cast(X, tf.int32), [-1])
        base_cov = (self.variance * tf.matmul(self.feature_mat, Z, adjoint_b=True) + self.offset)**self.degree
        cov = tf.sparse.sparse_dense_matmul(self.sparse_P, base_cov)
